...
def readVideo():
    ...
    inpImage = cv2.VideoCapture(os.path.join(CWD_PATH, 'resources/1.mp4'))
...
def general_search(binary_warped, left_fit, right_fit):
    ...
    cv2.fillPoly(window_img, [np.array(left_line_pts, dtype=int)], (0, 255, 0))
    cv2.fillPoly(window_img, [np.array(right_line_pts, dtype=int)], (0, 255, 0))
...
def draw_lane_lines(original_image, warped_image, Minv, draw_info):
    ...
    cv2.fillPoly(color_warp, [np.array(pts, dtype=int)], (0, 255, 0))
    cv2.fillPoly(color_warp, [np.array(pts_mean, dtype=int)], (0, 255, 0))
...
def addText(img, radius, direction, deviation, devDirection, current_wheel_position):
    ...
    cv2.putText(img, text2, (50, 200), font, 0.8, (0, 100, 200), 2, cv2.LINE_AA)

    # Deviation
    deviation_text = 'Off Center: ' + str(round(abs(deviation), 3)) + 'm' + ' to the ' + devDirection
    cv2.putText(img, deviation_text, (50, 250), cv2.FONT_HERSHEY_TRIPLEX, 0.8, (0, 100, 200), 2, cv2.LINE_AA)
...
def get_wheel_position(frame, previous_position, max_wheel_angle):
    ...
    Args:
        frame: 1280x720 resolution frame
    ...
    if np.sum(threshL) / 255 >= 10000 and np.sum(threshR) / 255 >= 10000:
        deviation, directionDev = offCenter(meanPts, frame)

        max_deviation_meters = 3.7 / 2
        ...
        return wheel_angle
    # if LeftBirdView has less that 10 000 piexels means left line doesn't exists
    elif np.sum(threshL) / 255 < 10000:
        return -max_wheel_angle
    elif np.sum(threshR) / 255 < 10000:
        return max_wheel_angle
    else:
        return previous_position


def applyToFrameDebug(frame, initialWheelPos):
    # # frame = cv2.imread("resources/testing_images/right_line.jpg", cv2.IMREAD_COLOR)
    # # frame = cv2.resize(frame, (1280, 720))
    ...
    initialWheelPos = get_wheel_position(frame, initialWheelPos, 20)
    # print(wheel_position)
    # cv2.waitKey(0)

    # Adding text to our final image
    # finalImg = addText(result, curveRad, curveDir, deviation, directionDev, initialWheelPos)
    return initialWheelPos, frame
...
#
#         # Adding text to our final image
#         finalImg = addText(result, curveRad, curveDir, deviation, directionDev, wheel_position)
#
